...
* **Prioritise colored mermaid diagrams wherever possible — a picture is worth a thousand words while explaining concepts.** A simple dependency or sequencing diagram of the sub-tasks is often clearer than a long bulleted list.
* Color the diagrams using `classDef` so intent is visually obvious (FE / BE / shared, blocking / parallel / optional, etc.). Keys: `fill` (background), `stroke` (border), `color` (text). Color edges with `linkStyle 0 stroke:#666CFF,stroke-width:2px`. Example:
